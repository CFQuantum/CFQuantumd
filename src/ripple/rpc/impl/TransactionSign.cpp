    ...
    std::shared_ptr<ReadView const>& ledger)
{
    ...
    bool isActivate = !isPayment &&
                           tx.isMember (jss::TransactionType) &&
                           tx[jss::TransactionType].asString () == "Activate";
    if (isActivate || isPayment)
    {
        if (!tx.isMember (jss::Destination))
        {
            return RPC::missing_field_error ("tx_json.Destination");
        }
        
        auto const dstAccountID = parseBase58<AccountID> (
            tx[jss::Destination].asString ());
        if (!dstAccountID)
            return RPC::invalid_field_error ("tx_json.Destination");
        ...
        //if currency is native(XRP), charge 1/1000 of transfer amount,
