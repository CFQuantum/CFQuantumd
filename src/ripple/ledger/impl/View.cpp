...
{
    STAmount amount;
    bool const bXRS (isXRS (currency));
    if (bXRS || isXRP(currency))
    {
        ...
        auto balance =
            sle->getFieldAmount(bXRS?sfBalanceXRS:sfBalance).xrp ();
        if (balance < reserve)
            amount.clear ();
        else
        {
...
void forEachItem (ReadView const& view, Keylet const root, 
    ...
    beast::Journal j)
{
    ...
    assert (!isXRS (uSenderID) && uSenderID != noAccount());
    assert (!isXRS (uReceiverID) && uReceiverID != noAccount());
    ...
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    ...
    assert (!isXRS (uSenderID) && !isXRS (uReceiverID));
    assert (uSenderID != uReceiverID);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    /* XRP or XRS send which does not check reserve and can do pure adjustment.
    ...
    if (ShouldLog (lsTRACE, View))
    {
        ...
        bool bXRS = isXRS(saAmount);

        if (sender)
            sender_bal = sender->getFieldAmount (bXRS?sfBalanceXRS:sfBalance).getFullText ();

        if (receiver)
            receiver_bal = receiver->getFieldAmount (bXRS?sfBalanceXRS:sfBalance).getFullText ();
    ...
    bool const bXRS (isXRS(saAmount));

    if (sender)
    {
        if (sender->getFieldAmount (bXRS?sfBalanceXRS:sfBalance) < saAmount)
        {
        ...
        else
        {
            ...
            sender->setFieldAmount (bXRS?sfBalanceXRS:sfBalance,
                sender->getFieldAmount (bXRS?sfBalanceXRS:sfBalance) - saAmount);
            view.update (sender);
        }
    }
    ...
    if (tesSUCCESS == terResult && receiver)
    {
        ...
        receiver->setFieldAmount (bXRS?sfBalanceXRS:sfBalance,
            receiver->getFieldAmount (bXRS?sfBalanceXRS:sfBalance) + saAmount);
    ...
    beast::Journal j)
{
    ...
    auto const& field = isXRS (amount) ? sfBalanceXRS : sfBalance;
