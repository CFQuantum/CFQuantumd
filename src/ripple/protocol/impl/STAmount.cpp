...
STAmount::STAmount (SField const& name, bool isXRS, std::int64_t mantissa)
    : STBase (name)
    , mIssue(isXRS?xrsIssue():xrpIssue())
    , mOffset (0)
    , mIsNative (true)
{
    set (mantissa);
...
STAmount::STAmount (SField const& name, bool isXRS,
...
STAmount::STAmount (XRPAmount const& amount)
    : mIssue(amount.isXRS()?xrsIssue():xrpIssue())
    , mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < zero)
{
...
STAmount operator+ (STAmount const& v1, STAmount const& v2)
{
    ...
    if (v1.native ())
        return { v1.getFName (), isXRS(v1), getSNValue (v1) + getSNValue (v2) };
...
STAmount::add (Serializer& s) const
{
    if (mIsNative)
    {
        ...
        if (!mIsNegative)
            if (isXRS(*this))
                s.add64 (mValue | cXRSNative | cPosNative);
            else
                s.add64 (mValue | cPosNative);
        else
            if (isXRS(*this))
                s.add64 (mValue | cXRSNative);
            else
                s.add64 (mValue);
    }
    else
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    bool const native = ! currency.isString () ||
		...
		(currency.asString() == systemCurrencyCode() || currency.asString() == systemCurrencyCodeXRS());
...
/*    {
    ...
    else if (isXRS(issue.currency))
        issue.account = xrsAccount();
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
	...
	if (v1.native() && v2.native() && isNative(issue))
    {
        ...
        return STAmount(v1.getFName (), isXRS(v1), minV * maxV);
    }
