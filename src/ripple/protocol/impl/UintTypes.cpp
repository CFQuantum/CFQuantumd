...
std::string to_string(Currency const& currency)
{
	...
	if (currency == xrsCurrency())
		return systemCurrencyCodeXRS();
    ...
    if ((currency & sIsoBits).isZero ())
    {
        ...
        // is not allowed.
		if ((iso != systemCurrencyCode()) && (iso != systemCurrencyCodeXRS()) &&
...
bool to_currency(Currency& currency, std::string const& code)
{
	...
	if (code.empty() || !code.compare(systemCurrencyCodeXRS()))
	{
		currency = xrsCurrency();
		return true;
	}
