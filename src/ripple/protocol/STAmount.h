...
public:
    ...
    static const std::uint64_t cXRSNative  = 0x2000000000000000ull;
    ...
    STAmount (SField const& name, bool isXRS, std::int64_t mantissa);

    STAmount (SField const& name, bool isXRS = false,
...
inline bool isXRS(STAmount const& amount)
{
	return isXRS(amount.issue().currency);
}
inline bool isNative(STAmount const& amount)
{
    return isXRP(amount) || isXRS(amount);
}
