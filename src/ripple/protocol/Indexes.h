...
uint256
getQuantumLinkIndex (AccountID const& low, AccountID const& high);

uint256
getQuantumDirIndex (AccountID const& account);
...
/** An link between 2 accounts */
struct link_t
{
    Keylet operator()(uint256 const& key) const
    {
        return { ltQUANTUM_LINK, key };
    }
};
static link_t const link {};
...
// The root page of account's quantum links
Keylet quantumDir (AccountID const& id);
