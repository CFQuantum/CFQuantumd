...
#include <ripple/app/tx/impl/Activate.h>
...
invoke_preflight (PreflightContext const& ctx)
{
    ...
    {
    case ttISSUE:           return Dividend         ::preflight(ctx);
    case ttACTIVATE:        return Activate         ::preflight(ctx);
...
invoke_preclaim (PreclaimContext const& ctx)
{
    switch(ctx.tx.getTxnType())
    {
    case ttISSUE:           return invoke_preclaim<Dividend>(ctx);
    case ttACTIVATE:        return invoke_preclaim<Activate>(ctx);
...
invoke_calculateBaseFee(PreclaimContext const& ctx)
{
    switch (ctx.tx.getTxnType())
    {
    case ttISSUE:           return Dividend::calculateBaseFee(ctx);
    case ttACTIVATE:        return Activate::calculateBaseFee(ctx);
...
invoke_apply (ApplyContext& ctx)
{
    switch(ctx.tx.getTxnType())
    {
    case ttISSUE:        { Dividend      p(ctx); return p(); }
    case ttACTIVATE:        { Activate      p(ctx); return p(); }
