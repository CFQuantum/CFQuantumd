...
LedgerConsensusImp::Type LedgerConsensusImp::s_type = LedgerConsensusImp::Type::Ripple;
#if USE_ZOOKEEPER
...
bool LedgerConsensusImp::isZkBasedConsensus ()
{
    int zkPeers = 0;
    app_.overlay ().foreach (
        [&](Peer::ptr const& peer) {
            if (peer->getConsensusType () == Type::ZooKeeper &&
                (peer->getClosedLedgerHash () == getLCL () ||
                 peer->hasLedger (getLCL (), mPreviousLedger->info ().seq)))
                ++zkPeers;
        });

    JLOG (j_.debug) << "Got " << zkPeers << " peers with ZooKeeper based consensus.";
    if (zkPeers >= 2)
        return true;
    return false;
}
...
bool LedgerConsensusImp::haveConsensus ()
{
    ...
    if (s_type == Type::ZooKeeper)
    {
        if (isZkBasedConsensus ())
        {
            ...
            auto path = boost::str (pathFmt % s_zkPath % (mPreviousLedger->info ().seq + 1));

            auto value = boost::str (valueFmt % to_string (mOurPosition->getCurrentHash ()) %
                                     to_string (getLCL ()) %
        ...
        {
            JLOG (j_.debug) << "Fallback to ripple consensus.";
        }
    }
#endif
...
#if USE_ZOOKEEPER
    if (s_type == Type::ZooKeeper && isZkBasedConsensus ())
    {
