        ...
        family))
{
    ...
    auto const id = *parseBase58<AccountID>("cDop6BbtxA5SmGahAtM741Ruf6cwke67MY");
    auto const sle = std::make_shared<SLE>(keylet::account(id));
    sle->setFieldU32 (sfSequence, 1);
    sle->setAccountID (sfAccount, id);
    sle->setFieldAmount (sfBalance, info_.drops);
    sle->setFieldU32 (sfQuantumLinksCount, 1);
    rawInsert(sle);
    ...
    , fees_(prevLedger.fees_)
{
    ...
    info_.dropsXRS = prevLedger.info().dropsXRS;
...
void Ledger::updateHash()
{
    ...
    info_.hash = sha512Half(
        ...
        std::uint64_t(info_.dropsXRS.drops ()),
        info_.parentHash,
        info_.txHash,
        info_.accountHash,
    ...
    Application& app, std::shared_ptr<Ledger> const& ledger, bool current)
{
    ...
    static boost::format addLedger (
        ...
        "(LedgerHash,LedgerSeq,PrevHash,TotalCoins,TotalCoinsXRS,ClosingTime,PrevClosingTime,"
    ...
    if (app.getTxnDB ().getType () != DatabaseCon::Type::None)
    {
        ...
        *db << boost::str (deleteTrans2 % seq);
        ...
        for (auto const& vt : aLedger->getMap ())
        {
            ...
            if (vt.second->getTxnType () == ttISSUE)
                continue;
...

    {
        ...
        *db << boost::str (
            ...
            to_string (ledger->info().drops) % to_string (ledger->info().dropsXRS) % ledger->info().closeTime %
...
loadLedgerHelper(std::string const& sqlSuffix, Application& app)
{
    ...
    boost::optional<std::string> sLedgerHash, sPrevHash, sAccountHash,
        sTransHash;
    boost::optional<std::uint64_t> totDrops, totDropsXRS, closingTime, prevClosingTime,
        closeResolution, closeFlags, ledgerSeq64;

    std::string const sql =
            "SELECT "
            "LedgerHash, PrevHash, AccountSetHash, TransSetHash, "
            "TotalCoins, TotalCoinsXRS, "
