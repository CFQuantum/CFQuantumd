    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        else
        {
            ...
            if (pathState_.inReq() >= zero)
            {
				...
				// Limit XRS by available. No limit for non-XRS as issuer.
				if (isXRS(node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            xrsCurrency(),
                            xrsAccount(),
                            fhIGNORE_FREEZE, viewJ)); // XRS can't be frozen
            ...
            else
            {
                ...
                resultCode = accountSend(view(),
                    node().account_, isXRP(node().issue_)?xrpAccount():xrsAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
