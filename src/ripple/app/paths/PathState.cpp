    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        resultCode = pushNode (type, isXRP(currency)?xrpAccount():xrsAccount(), currency, issuer);
    }


    // For ripple, non-XRP && non-XRS, ensure the issuer is on at least one side of the
    // transaction.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isXRP(node.issue_) ? xrpAccount() :
                 (isXRS(node.issue_) ? xrsAccount() : account));
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isXRP(node.issue_.currency) ? xrpAccount() :
                  (isXRS(node.issue_.currency) ? xrsAccount() : account));
    ...
    else
    {
        ...
        else if (isXRS (node.issue_.currency))
            node.issue_.account = xrsAccount();
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isXRP(uMaxCurrencyID) ? xrpAccount() :
            (isXRS(uMaxCurrencyID) ? xrsAccount() : uSenderID);
    // Sender is always issuer for non-XRP && non-XRS.
    ...
    // XRP or XRS with issuer is malformed.
    if ((isXRP (uMaxCurrencyID) && !isXRP (uMaxIssuerID))
        || (isXRP (currencyOutID) && !isXRP (issuerOutID))
		|| (isXRS (uMaxCurrencyID) && !isXRS (uMaxIssuerID))
		|| (isXRS (currencyOutID) && !isXRS (issuerOutID)))
    {
        JLOG (j_.debug)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        const auto nextAccountID   = spSourcePath.size ()
                ...
                : !isXRP(currencyOutID)
                ? (isXRS(currencyOutID) ? xrsAccount() : (issuerOutID == uReceiverID)
    ...
    if (terStatus == tesSUCCESS
        && !isNative(currencyOutID)            // Next is not XRP or XRS
    ...
    if (terStatus == tesSUCCESS)
    {
        ...
        terStatus   = pushNode (
			!isXRP(currencyOutID) && !isXRS(currencyOutID)
                ? STPathElement::typeAccount | STPathElement::typeCurrency |
                   STPathElement::typeIssuer
               : STPathElement::typeAccount | STPathElement::typeCurrency,
