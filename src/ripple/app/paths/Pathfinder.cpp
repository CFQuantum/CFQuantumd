    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isXRP(uSrcCurrency) ?
                xrpAccount() : (isXRS(uSrcCurrency) ? xrsAccount(): uSrcAccount))}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isXRP(uSrcCurrency) == isXRP(uSrcIssuer.get()) || isXRS(uSrcCurrency) == isXRS(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsXRS = isXRS (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsXRP && !isXRP (*mSrcIssuer) && !currencyIsXRS && !isXRS (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsXRP ? AccountID() : ( currencyIsXRS ? xrsAccount() : account );
    ...
    bool bSrcXrs = isXRS (mSrcCurrency);
    bool bDstXrs = isXRS (mDstAmount.getCurrency());
    ...
    if ((bSrcXrp && bDstXrp) || (bSrcXrs && bDstXrs))
    {
        // XRP -> XRP
        JLOG (j_.debug) << "XRP to XRP payment";
        paymentType = pt_XRP_to_XRP;
    }
    else if (bSrcXrp && bDstXrs)
    {
        // XRP -> XRS
        JLOG (j_.debug) << "XRP to XRS payment";
        paymentType = pt_XRP_to_XRS;
    }
    else if (bSrcXrp)
    {
        // XRP -> non-XRP
    ...
    else if (bDstXrp && bSrcXrs)
    {
        // XRS -> XRP
        JLOG (j_.debug) << "XRS to XRP payment";
        paymentType = pt_XRS_to_XRP;
    }
    else if (bDstXrp)
    {
        // non-XRP -> XRP
    ...
    {
        // XRS -> non-XRP
        JLOG (j_.debug) << "XRS to non-XRP payment";
        paymentType = pt_XRS_to_nonXRP;
    }
    else if (bDstXrs)
    {
        // non-XRP -> XRS
        JLOG (j_.debug) << "non-XRP to XRS payment";
        paymentType = pt_nonXRP_to_XRS;
    }
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isXRP (mSrcCurrency) || isXRS (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    int addFlags)
{
	...
	bool const bOnXRS = isXRS(uEndCurrency);
        ...
        else
        {
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    bool bXRS = isXRS(book->getCurrencyOut());
                    
                    if (book->getCurrencyOut().isZero() || bXRS)
                    { // to XRP

                        // add the order book itself
                        newPath.emplace_back (
                            STPathElement::typeCurrency,
                            bXRS?xrsAccount ():xrpAccount (),
                            bXRS?xrsCurrency ():xrpCurrency (),
                            bXRS?xrsAccount ():xrpAccount ());

                        if (bXRS ?
                            isXRS (mDstAmount.getCurrency ()):
